...
| Merknad / _Note_ | Utvidelse sammenlignet med DCAT-AP-NO: Kravnivået er endret fra obligatorisk til anbefalt, tilsvarende med multiplisitet fra 1..n til 0..n. Dette fordi ikke alle har beskrivelse på blokknivå som metadata. 
...
| Merknad 1 / _Note 1_ | Verdien bør velges fra kontrollerte vokabularer, f.eks. EUs kommende kontrollerte vokabularer for livshendelser, sektorspesifikke kodeverk (f.eks. SNOMED CT) osv. 

_The value should be chosen from controlled vocabularies, e.g. EU's coming controlled vocabularies for live events, sector specific classifications (e.g. SNOMED CT) etc._
